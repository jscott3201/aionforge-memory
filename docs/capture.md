...
A writer that *knows* its capture replaces an earlier memory can say so: `CaptureRequest.supersedes` (the `supersedes` parameter on the MCP `capture` tool) names a live episode the new content corrects or replaces. This is the writer-asserted half of the spec's add/update/supersede step (04 §1 step 3), and it stays evidence, not action: capture validates the claim, records it in the new episode's `Origin.supersedes`, and echoes it on the receipt (`sup=<id>` on the MCP receipt line) — it never retires the target. Consolidation's supersession pass consumes the hint and decides what, if anything, it retires, under the same trust rules as every other supersession — see [consolidation](consolidation.md) for the exact rules (fact overlap, at-least-equal trust, one survivor per pair).

Validation is strict and oracle-free. The target must be a **live** episode (a soft-forgotten one is already out of recall) in a namespace the writer could itself write — checked with the same authorizer as the write. A missing id, an expired target, and a target in someone else's namespace all refuse the capture with the **same** error, so the hint cannot be used to probe which ids exist; the specific cause is recorded only in a `SupersedesRejected` audit. When the content turns out to be an exact duplicate, dedup wins and the hint is dropped — the existing episode is immutable — and the `ExactDuplicate` verdict on the receipt tells the writer the claim went nowhere.
...
A *rejected* write is audited too, in its own transaction through `Store::commit_audit`, and produces no memory node. A namespace denial writes a `NamespaceDenied` event whose subject is the agent itself (there is no episode to point an `AUDIT` edge at). A signed-write rejection writes an `InvalidSignature` or `ClockSkewRejected` event. A residue-only refusal writes a `ResidueRejected` event, likewise subject-on-the-agent, as does an invalid supersedes hint (`SupersedesRejected`, carrying the claimed target id and the specific cause the returned error deliberately collapses). These standalone audits are content-addressed on their own id, so a deterministic retry of the same rejected attempt is a no-op rather than a spurious constraint error.
