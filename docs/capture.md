...
- **Injection stripping.** Known prompt-injection markers (override phrases like "ignore previous instructions", "system prompt:", "you are now") are detected, stripped from the content, and their ids collected.
...
The v1.0 default pattern set is **deliberately conservative**: a small, low-false-positive starting set that raises the bar rather than claiming to catch everything. A host that needs more supplies its own redaction and marker rules through `CaptureFilter::new`. This is not a content classifier and not a guarantee against a determined exfiltration attempt; it is a cheap, deterministic first scrub on the hot path.
...
A *rejected* write is audited too, in its own transaction through `Store::commit_audit`, and produces no memory node. A namespace denial writes a `NamespaceDenied` event whose subject is the agent itself (there is no episode to point an `AUDIT` edge at). A signed-write rejection writes an `InvalidSignature` or `ClockSkewRejected` event. These standalone audits are content-addressed on their own id, so a deterministic retry of the same rejected attempt is a no-op rather than a spurious constraint error.
