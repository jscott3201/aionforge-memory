...
| OAuth resource server | HTTP auth is default-off and local mode still relies on explicit MCP identity parameters. When `[auth].enabled=true`, `aionforge serve http` validates bearer tokens for `/mcp`, maps verified claims to an authoritative principal, and serves RFC 9728 protected-resource metadata. | Full remote deployments must either enable the built-in HTTP OAuth posture or add an upstream OAuth-aware verifier/equivalent perimeter. |
