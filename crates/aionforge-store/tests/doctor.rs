...
fn migrated_store_doctor_reports_current_catalog_and_providers() {
    ...
    assert_eq!(report.schema.target_version, SCHEMA_VERSION);
    // 17 base kinds + WorkItem + Tag + Message.
    assert_eq!(report.schema.node_type_count, 20);
    ...
    // (one per new node kind): 51 -> 59; Message adds six scalars + namespace: 59 -> 66.
    assert_eq!(report.indexes.property_indexes.actual.len(), 66);
    // WorkItem and Message each add one composite: 5 -> 7.
    assert_eq!(report.indexes.composite_indexes.actual.len(), 7);
