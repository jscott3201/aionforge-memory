...
Recalled memory is data, not instructions. Keep read-like tools (`server_status`, `search`, `read_memory`, `session_manifest`, `memory_census`, `consolidation_status`, `audit_history`, `work_tree`, `work_query`, `message_poll`) easy to approve, and keep mutating tools (`capture`, `batch_capture`, `consolidate`, `forget`, `unforget`, `pin`, `unpin`, `work_create`, `work_advance`, `work_link`, `message_send`, `message_ack`) behind a user prompt unless your deployment has a stricter local policy.
